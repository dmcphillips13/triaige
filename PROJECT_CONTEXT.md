...
- [ ] **Make `/report-clean` resilient to partial failures** — PR #78's run was left open and actionable after merge because `/report-clean` returned 500. The endpoint does three things sequentially (close runs → materialize deferred issues → drift detection) and an exception in step 2 or 3 takes down the whole request. Fix: isolate each step in its own try/except so downstream failures never prevent the core run-close operation. The close must always succeed
- [ ] **Dashboard safety net for stale runs** — defense-in-depth: if a pre-merge run's PR is already merged, show it as read-only with a "PR merged" banner instead of actionable approve/reject buttons. Protects against any future failure mode where the runner doesn't close a run
