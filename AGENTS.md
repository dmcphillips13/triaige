...
26. Repo setup CLI (`npx triaige init`): guided setup run inside a target repo; checks for `gh` CLI and auth (if missing, guides user to install and run `gh auth login`; falls back to manual `.env.example` if `gh` unavailable); verifies repo is connected on the Triaige dashboard; prompts for API key and runner URL (copied from dashboard settings), sets them as GitHub repo secrets via `gh secret set`; detects existing Playwright config or scaffolds new config; prompts for app build/start command and wires it into the workflow; scaffolds `.github/workflows/visual-regression.yml`, `scripts/post-failures.sh`, and `.env.example` as reference; detects monorepo structure and adjusts paths; offers to generate initial baselines (`npx playwright test --update-snapshots`) if app can be started locally
