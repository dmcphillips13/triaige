...
- [ ] Step 22: Issue attribution agent — separate async agent that runs after issues are filed; scans recent merge history and Playwright workflow artifacts to attribute failures to the responsible PR and author; builds audit trail for filtered-out failures so no failure goes unrecorded
