...
20.1. Filter post-merge runs to net-new failures only: compare each failure against previous open post-merge runs for the same repo; exclude failures that already exist in a prior run; if zero new failures remain, skip run creation entirely (no empty runs on dashboard); runs can only be closed when all failures are addressed — every failure must have a baseline update PR or issue opened before close is allowed
20.2. Collapse known failures in PR comment: pre-merge PR comments still show all failures but restructure to lead with new failures (the signal) and collapse known failures into a `<details>` block; known failures should clearly indicate they were not caused by this PR
20.3. Prior-PR attribution for unexplained failures: when a new failure on a post-merge run can't be explained by the current PR's changes, scan recent merged PR descriptions (from the `runs` table) and include attribution in the classifier rationale ("likely introduced by PR #X"); include this attribution in GitHub issues created for rejected failures so the responsible developer can find and address issues caused by their changes
