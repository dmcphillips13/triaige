...
22. Issue attribution agent: a separate async agent that runs after issues are filed; scans recent merge history and Playwright workflow artifacts to attribute failures to the responsible PR and author; adds attribution comments to issues; also builds an audit trail for failures that were filtered out of a run (still failing but already tracked on an earlier run) so no failure goes unrecorded
23. Auto-approve baselines above confidence threshold (stretch — needs discussion before implementing; default should always be human-in-the-loop)
