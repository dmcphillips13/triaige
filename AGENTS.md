...
21. Separate test repo support: allow users to link a UI repo (source of PR context) with a separate Playwright/baselines repo (source of tests and baseline screenshots); triage runs use PR context from the UI repo but baseline update PRs target the test repo; the UI repo workflow checks out both repos, runs Playwright from the test repo against the built UI, and posts results to Triaige; keeps UI repos clean of test infrastructure and screenshot bloat; dashboard replaces single-repo linking with a "repo pair" model (UI repo + test repo)
...
25. Procedural memory: agent reflection on feedback patterns → self-updating triage instructions, versioned in Qdrant (stretch)
26. Settings UI: per-repo configuration page in dashboard for API keys (OpenAI, etc.), triage mode toggles, and other config that currently lives in env vars or is hardcoded
