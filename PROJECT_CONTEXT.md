...
- [ ] Step 22: Classification improvements — observed issues and fixes:
  - **Mixed-scope contamination**: when a page has both expected changes (font sizes) and unexpected changes (sidebar color), the classifier compromises to "uncertain" instead of separating them. Fix: feed the git diff so the classifier can match each visual change to a specific CSS property/component and classify per-change, not per-page.
  ...
  - **Git diff context**: the biggest lever. If the classifier sees `--font-size-2xl` changed AND `--color-bg-sidebar` changed, it can say "font size change on overview = expected, sidebar color change = unexpected" instead of averaging to uncertain.
  - **Prompt tuning**: add explicit rules: (1) if all visual changes on a page trace to CSS properties in the diff AND the PR description mentions those properties, classify as expected; (2) if any visual change on a page traces to a CSS property NOT mentioned in the PR description, that specific change is unexpected; (3) uncertain only when the scope alignment is genuinely ambiguous.
