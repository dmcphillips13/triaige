...
- [ ] Step 20.1: Filter post-merge runs to net-new failures only — compare against previous runs and exclude failures that already exist; if zero new failures, skip run creation entirely; runs can only be closed when all failures are addressed (baseline PR or issue opened)
- [ ] Step 20.2: Collapse known failures in PR comment — pre-merge comments still show all failures but collapse known ones into a `<details>` block; new failures are the signal, known failures clearly indicate they were not caused by this PR
- [ ] Step 20.3: Prior-PR attribution — for new failures not explained by the current PR, scan recent merged PR descriptions and include attribution in the classifier rationale ("likely introduced by PR #X"); include this attribution in GitHub issues so the responsible developer can find issues caused by their changes
