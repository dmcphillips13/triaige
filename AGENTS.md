...
20.1. Auto-close superseded runs: when a new post-merge run arrives for the same repo, automatically close the previous open post-merge run (prevents stale runs accumulating in Main tab)
20.2. Collapse known failures in PR comment: restructure the PR comment to lead with new failures (the signal), then collapse known failures into a `<details>` block; known failures should clearly indicate they were not caused by this PR
20.3. Fix post-merge classification: push-triggered runs extract PR number from merge commit but may lose PR description context, causing all failures to classify as unexpected; ensure classifier receives the full PR description and changed files for accurate expected/unexpected classification
