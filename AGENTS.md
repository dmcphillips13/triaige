...
21. Actionable pre-merge workflow with merge gate:
    ...
    - **Merge gate (on by default)**: a GitHub check status stays pending until every net-new failure has either a baseline commit or an issue; pre-existing failures (already failing on main with an open issue) do not count toward the gate; gate can be turned off per-repo in settings
...
26. Settings UI: per-repo configuration page in dashboard for API keys (OpenAI, etc.), triage mode toggles, merge gate toggle, and other config that currently lives in env vars or is hardcoded
