...
- [ ] Step 22: Classification improvements — reduce LLM temperature to 0 for deterministic results; feed git diff / changed file paths into classifier for stronger scope signal; tune prompt with more explicit classification rules (e.g., "if changed file is in PR diff, default to expected unless visible defect"); leverage episodic memory few-shot examples more aggressively for calibration
