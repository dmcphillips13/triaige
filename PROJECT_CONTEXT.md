...
- [ ] Step 21.1: Fix submission label on pre-merge runs — should say "Baseline committed" not "Baseline PR opened" since baselines are committed directly to the PR branch
...
- [ ] Step 21.3: Classification improvements — reduce LLM temperature to 0 for deterministic results; feed git diff / changed file paths into classifier for stronger scope signal; tune prompt with more explicit classification rules (e.g., "if changed file is in PR diff, default to expected unless visible defect"); leverage episodic memory few-shot examples more aggressively for calibration
- [ ] Step 21.4: Main tab health dashboard redesign — show failing baselines with linked issues rather than the merge PR run card; redesign as a health dashboard of open issues on failing tests with ability to close/resolve directly from the UI
- [ ] Step 21.5: Known failure visibility on PRs — when net-new filtering skips known failures, post a PR comment listing skipped tests with links to their open issues and a note to verify manually; also show skipped tests as non-actionable in the dashboard run detail if a triage run exists
- [ ] Step 22: Baseline knowledge doc — LLM auto-generates semantic descriptions of each page from baseline screenshots and app code; classifier references during triage for richer context; enables component-level scope mapping (which components render on which pages) for deterministic in-scope vs out-of-scope flagging before LLM runs
